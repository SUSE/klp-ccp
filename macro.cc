...
macro::instance::instance(const std::shared_ptr<const macro> &macro,
  ...
  : _macro(macro),
    ...
    _pending_ws_state(_pending_ws_state::dont_emit),
...
pp_token macro::instance::_yield_pending_ws()
{
  assert(_anything_emitted);
  assert(_pending_ws_state == _pending_ws_state::emit_on_nonempty);
  // Obviously, the next token will be non-empty, because that's the
  // reason why we're emitting the pending space now.
  _pending_ws_state = _pending_ws_state::nonempty_seen;
  return pp_token(pp_token::type::ws, " ", _file_range);
}

void macro::instance::_report_ws() noexcept
{
  if (_pending_ws_state == _pending_ws_state::nonempty_seen)
    _pending_ws_state = _pending_ws_state::emit_on_nonempty;
}
...
pp_token macro::instance::_yield_concat_token()
{
  ...
  if (!_concat_token->is_empty()) {
    if (_pending_ws_state == _pending_ws_state::emit_on_nonempty)
...
pp_token macro::instance::read_next_token()
{
  ...
  //   token
  // - Whitespace tokens are allowed only inbetween the first and last
  //   non-empty tokens, if any.
  ...
  handle_arg:
	...
	_add_concat_token(*_cur_arg_it++);
      ...
      // The remaining tokens, if any, are not all ws
      assert(_cur_arg_it == _cur_arg->cend() ||
	     !std::all_of(_cur_arg_it, _cur_arg->end(),
			  [](const pp_token &tok) {
			    return tok.is_ws();
			  }));
	...
	++_cur_arg_it;
      ...
      // result.
      while(_cur_arg_it != _cur_arg->end()) {
	...
	_add_concat_token(*_cur_arg_it++);
      ...
      if (_cur_arg_it->is_ws()) {
	_report_ws();
	assert(_cur_arg_it + 1 != _cur_arg->end());
	++_cur_arg_it;
	assert(!_cur_arg_it->is_ws() && !_cur_arg_it->is_empty());
      }

      if (!_cur_arg_it->is_empty()) {
	if (_pending_ws_state == _pending_ws_state::emit_on_nonempty)
	  return _yield_pending_ws();
	_pending_ws_state = _pending_ws_state::nonempty_seen;
      }
  ...
  if (_it_repl->is_ws()) {
    _report_ws();
    ++_it_repl;
    assert(_it_repl != _macro->_repl.end() && !_it_repl->is_ws());
  }

  assert(!_it_repl->is_empty());
  if (_pending_ws_state == _pending_ws_state::emit_on_nonempty)
    return _yield_pending_ws();
  _pending_ws_state = _pending_ws_state::nonempty_seen;
