...
static const _val_tok_map_entry kw_map_entries[] = {
  ...
  { "_Generic", gnuc_parser::token_type::TOK_KW_GENERIC },
...
gnuc_parser_driver::gnuc_parser_driver(preprocessor &&pp,
  ...
  : _result(nullptr), _pp(std::move(pp)),
    ...
    _ext_float_keywords(tgt.get_ext_float_keywords())
{
			...
			gnuc_parser::location_type *loc)
{
  ...
  case pp_token::type::id:
    ...
    {
      const auto it_ext_int_kw = _ext_int_keywords.find(tok.get_value());
    ...
    {
      const auto it_ext_float_kw = _ext_float_keywords.find(tok.get_value());
      if (it_ext_float_kw != _ext_float_keywords.cend()) {
	value->ext_float_kind = it_ext_float_kw->second;
	return gnuc_parser::token_type::TOK_KW_EXT_FLOAT;
      }
    }
